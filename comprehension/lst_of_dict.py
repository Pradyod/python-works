...
# all_mlym_movies=[m.get('title') for m in movies if m.get('language')=="Malayalam"]
# print(all_mlym_movies)


all_yr=[m.get("year") for m in movies if m.get("year")]
# print(all_yr)

yr_c={m:all_yr.count(m) for m in all_yr}
print(yr_c)

max_yr=max(yr_c,key=yr_c.get)
print(max_yr)
